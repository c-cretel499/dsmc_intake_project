...
public:
	...
	void advanceKernel(size_t p_start, size_t p_end, World& world, std::vector<Particle>& particles, double qm, double dt );			//Function call for each thread
	void computeGasProperties();	// computes density and velocity
	void sortParticlesinParallel(int p_start, int p_end, World& world, std::vector<std::vector<int>>& cell_part_ids);
