...
int DSMC_MEX::perform()
{
	...
	int n_threads = world.num_threads;		//Max number of threads possible
	size_t np = num_cells;
	size_t np_per_thread = np / n_threads;	//Particles per thread
	std::vector<std::thread> threads;

	for (int i = 0; i < n_threads; i++) {
		size_t p_start = i * (np_per_thread);
		size_t p_end = (i + 1) * (np_per_thread);

		if (i = n_threads - 1) p_end = np;

		threads.emplace_back(&DSMC_MEX::parallelPerform, p_start, p_end, &num_cols);
	}
...
void DSMC_MEX::parallelPerform(int c_start, int c_end, int* num_cols)
{
	...
	if (num_cols) {
		sigma_vr_max = sigma_vr_max_temp;			///make this as array
	}
